 ...
 * THe keys of this array are the codes of the documents.
 * @param Array $languages the languages available in the corpus
 * @param Array $ngramdata An array for storing ngrams for furhter use in
 * calculating LL etc. Format:
 * [2=>[["this+bigram"]=>["freq"=>x],["another+bigram"]=>["freq"=>x]]]
...
class Corpus extends CorpusObject
{
    ...
    public function SetLanguages()
    {
        if (!$this->languages) {
            $result = pg_query_params(
                $this->corpuscon,
                "SELECT DISTINCT lang FROM librarysrc",
                array()
            );
    ...
    public function GetDocumentNamesAndCodes($lang)
    {
        $result = pg_query_params(
            $this->corpuscon,
            "SELECT DISTINCT code, title FROM librarysrc WHERE lang = $1",
            array($lang)
        );
    ...
    public function SetWordFrequenciesPerWholeCorpus()
    {
        ...
        foreach ($freqs as $row) {
            if ($row[$this->filter->target_col]) {
                $this->word_frequencies[$row[$this->filter->target_col]] =
                    $row["count"] * 1;
            }
        }
        return $this;
    ...
    public function SetNounFrequencyByLemma()
    {
        foreach ($this->documents as $doc) {
            $doc
                ->SetNounFrequencyByLemma($this->stopwords)
    ...
    public function CountGt2gramLL()
    {
        $this->data = array();
        foreach (
            $this->ngramdata[$this->ngram_number]
            as $ngram => $ngramdata
        ) {
            ...
            for ($i = 0; $i < $this->ngram_number - 1; $i++) {
                $thiskey = "{$words[$i]}{$this->ngram_separator}{$words[$i +
                        1]}";
                if (!array_key_exists($thiskey, $this->ngramdata[2])) {
                    $all_keys_found = false;
                } else {
                    $log_likelihoods_of_bigrams[] =
                        $this->ngramdata[2][$thiskey]["LL"];
                    $pmi_of_bigrams[] = $this->ngramdata[2][$thiskey]["PMI"];
                }
            }
            if (
                sizeof($log_likelihoods_of_bigrams) ==
                $this->ngram_number - 1
            ) {
                $this->data[] = array(
                    "ngram" => $ngram,
    ...
    private function GetWithoutSecond($words)
    {
        return $this->ngram_frequencies_by_first_word[$words[0]] -
            $this->ngram_frequencies[
                "$words[0]{$this->ngram_separator}$words[1]"
            ];
    }
    ...
    public function SetAddressesOfAllDocuments()
    {
        ...
        $i = 1;
        foreach ($this->documents as $doc) {
            $this->document_addresses["arr"] = array_merge(
                $this->document_addresses["arr"],
                $doc->GetAddr()
            );
            ...
            $next = $i + 1;
            $this->document_addresses[
                "str"
            ] .= "({$this->filter->id_col} BETWEEN \$$i AND \$$next)";
            $i += 2;
        }
    }
     ...
     *
     **/
    public function SetNgramMustIncludeWord(
        $word,
        $last_index,
        $n,
        $word_is_lemma = false
    ...
    public function SetStopWordFilterToNgramQuery($last_idx)
    {
        $indices = range(
            $last_idx + 1,
            $last_idx + 1 + sizeof($this->stopwords) - 1
        );
     ...
     *
     */
    public function SetNgramFrequency(
        $n,
        $min_count = 0,
        $include_word = "",
        $pos_array = array(),
        $word_condition_is_lemma = false
    ) {
        $this->SetAddressesOfAllDocuments();
        $this->ngram_number = $n;
        ...
        $wordcolstring = implode(
            " || '{$this->ngram_separator}' || ",
            $wordcols
        );
        $last_addr_index = sizeof($this->document_addresses["arr"]) + 1;
        $addr_condition = str_replace(
            $this->filter->id_col,
            "tokentab.{$this->filter->id_col}",
            $this->document_addresses["str"]
        );
        ...
        $filter_stop_words = $this->SetStopWordFilterToNgramQuery(
            $last_addr_index + 1
        );
        $filter_by_pos = "";
        $filter_by_word = "";
        ...
        if ($include_word) {
            $filter_by_word = $this->SetNgramMustIncludeWord(
                $include_word,
            ...
            $filter_by_word = $filter_by_pos
                ? " AND $filter_by_word AND "
                : " AND $filter_by_word ";
        }
            ...
            foreach ($freqs as $row) {
                if ($row["ngram"]) {
                    ...
                    if (
                        array_key_exists(
                            $words[0],
                            $this->ngram_frequencies_by_first_word
                        )
                    ) {
                        $this->ngram_frequencies_by_first_word[$words[0]] +=
                            $row["count"] * 1;
                    } else {
                        $this->ngram_frequencies_by_first_word[$words[0]] =
                            $row["count"] * 1;
    ...
    public function SetStopWords()
    {
        $result = pg_query_params(
            $this->maincon,
            "SELECT DISTINCT lemma FROM topicwords_stopwords",
            array()
        );
    ...
    public function AddNewStopWord($word)
    {
        $result = pg_query_params(
            $this->maincon,
            "INSERT INTO topicwords_stopwords (lemma) VALUES ($1)",
            array($word)
        );
        return $this;
    }
